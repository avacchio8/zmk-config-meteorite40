...
- rotary encoder UI を metadata-driven にする firmware には、`ConfigState.encoderSlots` を返す ZMK fork と `&met_enc` の `slots` property が必要です。
...
- overlay 側の `sensor-bindings` は `&met_enc` を参照し、`&met_enc` が slot binding を press/release します。
